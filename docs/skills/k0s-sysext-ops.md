...
The KubeStellar kiosk proxy TLS certificate (`cert.pem`) and private key (`key.pem`) are generated on first boot by `k0s-kiosk-tls.service`, which runs `/usr/share/k0s/kiosk-tls/generate-kiosk-tls.sh` (source: `files/k0s/kiosk-tls/generate-kiosk-tls.sh`) before `k0scontroller.service` starts. The private key is stored persistently in `/var/lib/k0s/kiosk/key.pem` with mode `0600` (the script runs under `umask 077`, so the key is never world-readable, even transiently), and the certificate is configured with the host's actual IP addresses in the Subject Alternative Names (SAN). `files/k0s/sysext/k0s-manifests.conf` seeds the kiosk static assets (`nginx.conf`, `kiosk-gate.js`, `kiosk-gate.css`) into this same directory on every boot, file-by-file rather than as a whole-directory copy, specifically so it never deletes `cert.pem`/`key.pem`.

**Compromised build-time material is replaced automatically.** Sysext releases prior to `k0s-kiosk-tls.service` generated the certificate and key at image build time and shipped them inside the publicly-published release image, so those private keys are public knowledge. Hosts upgraded from such a release still carry the compromised pair in `/var/lib/k0s/kiosk`. The generator detects it — the build-time certificate carries the fixed SAN `DNS:localhost,DNS:*.local,IP:127.0.0.1,IP:10.0.2.15` and lacks the `OU=host-generated` subject marker the script adds — and regenerates the pair on the next boot (or on `systemctl restart k0s-kiosk-tls.service`). Host-generated and operator-provided certificates are never touched.
